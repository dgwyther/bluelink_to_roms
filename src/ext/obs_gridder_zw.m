function gridded = obs_gridder_zw( data, opt )
...
% sort data structure to have ascending times
[time,it] = sort(data.time,'ascend');     
data=delstruct(data,it,length(data.time));

n=numel(data.lon);
x=reshape(data.lon,n,1);
y=reshape(data.lat,n,1);
xx=reshape(data.x,n,1);
yy=reshape(data.y,n,1);
...
haveerror=exist('ee','var'); 
if haveerror
err=reshape(data.error,n,1);
end
...
haveprov=exist('pp');
if haveprov
prov=reshape(data.provenance,n,1);
end
...
for s=1:length(search_list),
  ...
  for ts=1:length(dt)-1,
    ...
    if ( opt.is3d )
      %opt.ddepths=opt.z_w(:,round(mean(data.y(pts))),round(mean(data.x(pts))));
        opt.ddepths=opt.z_w(:,floor(mean(data.y(in)))+1,floor(mean(data.x(in)))+1);
          opt.ddepths = reshape(opt.ddepths, 1, length(opt.ddepths));
         l=find(opt.ddepths>0);
         if ( ~isempty(l) )
         opt.ddepths(l)=-opt.ddepths(l);
         end
         opt.ddepths = sort(opt.ddepths,'descend');clear l
      ...
      for z=2:length(opt.ddepths),
        ...
        if ( ~isempty(l) )
          ...
          gridded.x(count,1) = nanmean(xx(l));
          gridded.y(count,1) = nanmean(yy(l));
          gridded.num(count,1) = length(l);
            if haveerror
             gridded.error(count,1) = nanmean(err(l));
            end
            if haveprov
             gridded.provenance(count,1) = nanmean(prov(l));
            end
          for v=1:length(opt.vars)
      ...
      gridded.x(count,1) = nanmean(xx(in));
      gridded.y(count,1) = nanmean(yy(in)); 
      gridded.num(count,1) = length(in);
            if haveerror
             gridded.error(count,1) = nanmean(err(in));
            end
            if haveprov
             gridded.provenance(count,1) = nanmean(prov(in));
            end
      for v=1:length(opt.vars)
