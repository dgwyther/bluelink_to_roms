...
if nargi>0 && isscalar(varargin{1}) && ishandle(varargin{1})
   ...
   if strcmp(get(HAxes,'Tag'),'MMPOLAR_Axes')
      ...
      error('First Argument is Not a Valid MMPOLAR Axes Handle.')
   end
...
if nargi==0   % MMPOLAR() MMPOLAR() MMPOLAR() MMPOLAR() MMPOLAR() MMPOLAR()
   if ~isempty(HAxes)
      ...
      error('No MMPOLAR Axes exists or is not Current Axes.')
   end
end
if nargi==1   % Consider SET and GET Requests Consider SET and GET Requests
   if ~isempty(HAxes)
      ...
      error('No MMPOLAR exists or is not Current Axes.')
   end
end
...
if rem(nargi,2)==0 && ischar(varargin{1}) && ~isempty(HAxes)
   for k=1:2:nargi-1
      ...
      if ischar(PName)
         ...
         error('String Input Property Name Argument Expected.')
      end
   end
   local_updatePlot(HAxes,S)
   return
elseif ischar(varargin{1})      % Unknown Input Unknown Input Unknown Input
   error('Unknown Input Arguments or NO MMPOLAR Axes Exists.')
...
%--------------------------------------------------------------------------
function local_updatePlot(HAxes,S)
...
for k=1:length(Sfn)
   ...
   case 'Axis'                                                       % Axis
      ...
      if istrue
         ...
         error('Unknown ''Axis'' Property Value.')
      end
   case 'Border'                                                   % Border
      ...
      elseif istrue && strcmp(onoff,'off')
         ...
         error('Unknown ''Border'' Property Value.')
      end
   case 'BorderColor'                                         % BorderColor
      ...
      if istrue
         ...
         error('Unknown ''BorderColor'' Property Value.')
      end
   case 'Grid'                                                       % Grid
   ...
   case 'TickLength'                                           % TickLength
      if isnumeric(S.TickLength) && isscalar(S.TickLength)
         ...
         error('Unknown ''TickLength'' Property Value.')
      end
   case 'BackgroundColor'                                 % BackgroundColor
      ...
      if istrue
         ...
         error('Unknown ''BackgroundColor'' Property Value.')
      end
   case 'FontName'                                               % FontName
      if ischar(S.FontName) && any(strcmpi(listfonts,S.FontName))
         ...
         error('Unknown ''FontName'' Property Value.')
      end
   case 'FontSize'                                               % FontSize
      if isnumeric(S.FontSize) && isscalar(S.FontSize)
         ...
         error('Unknown ''FontSize'' Property Value.')
      end
   case 'FontWeight'                                           % FontWeight
      if ischar(S.FontWeight) && ...
         ...
         error('Unknown ''FontWeight'' Property Value.')
      end      
   case 'RGridColor'                                           % RGridColor
      ...
      if istrue
         ...
         error('Unknown ''RGridColor'' Property Value.')
      end
   case 'RGridVisible'                                       % RGridVisible
      ...
      if istrue
         ...
         error('Unknown ''RGridVisible'' Property Value.')
      end      
   case 'RGridLineStyle'                                   % RGridLineStyle
      if local_islinespec(S.RGridLineStyle)
         ...
         error('Unknown ''RGridLineStyle'' Property Value.')
      end               
   case 'RLimit'                                                   % RLimit
      if isnumeric(S.RLimit) && numel(S.RLimit)==2
         ...
         error('Unknown ''RLimit'' Property Value.')
      end               
   case 'RGridLineWidth'                                   % RGridLineWidth
      if isnumeric(S.RGridLineWidth) && isscalar(S.RGridLineWidth)
         ...
         error('Unknown ''RGridLineWidth'' Property Value.')
      end
   case 'RTickOffset'                                         % RTickOffset
      if isnumeric(S.RTickOffset) && isscalar(S.RTickOffset)
         ...
         for k=1:D.RTickLabelN
          ...
          error('Unknown ''RTickOffset'' Property Value.')
      end
   case 'RTickValue'                                           % RTickValue
      if isnumeric(S.RTickValue) && numel(S.RTickValue)>0
         ...
         error('Unknown ''RTickValue'' Property Value.')
      end
   case 'RTickVisible'                                       % RTickVisible
      ...
      if istrue
         ...
         error('Unknown ''RTickVisible'' Property Value.')
      end
   case 'RTickLabel'                                           % RTickLabel
      if iscellstr(S.RTickLabel)
         ...
         error('Unknown ''RTickLabel'' Property Value.')
      end
   case 'RTickLabelHalign'                               % RTickLabelHalign
      ...
      if ~isempty(out)
         ...
         error('Unknown ''RTickLabelHalign'' Property Value.')
      end         
   case 'RTickLabelValign'                               % RTickLabelValign
      ...
      if ~isempty(out)
         ...
         error('Unknown ''RTickLabelValign'' Property Value.')
      end
   case 'RTickLabelVisible'                             % RTickLabelVisible
      ...
      if istrue
         ...
         error('Unknown ''RTickLabelVisible'' Property Value.')
      end
   case 'RTickAngle'                                           % RTickAngle
      if isnumeric(S.RTickAngle) && isscalar(S.RTickAngle)
         ...
         elseif rad>P.TLimit(1) && rad<P.TLimit(2)
            ...
            error('RTickAngle not within Theta Axis Limits.')
         end
   ...
   case 'TGridColor'                                           % TGridColor
      ...
      if istrue
         ...
         error('Unknown ''TGridColor'' Property Value.')
      end
   case 'TDirection'                                           % TDirection
      ...
      elseif ischar(S.TDirection) && strcmpi(S.TDirection,'ccw')
         ...
         error('Unknown ''TDirection'' Property Value.')
      end
      P.Style='unknown';
   case 'TGridVisible'                                       % TGridVisible
      ...
      if istrue
         ...
         error('Unknown ''TGridVisible'' Property Value.')
      end      
   case 'TGridLineStyle'                                   % TGridLineStyle
      if local_islinespec(S.TGridLineStyle)
         ...
         error('Unknown ''TGridLineStyle'' Property Value.')
      end               
   case 'TLimit'                                                   % TLimit
      if isnumeric(S.TLimit) && numel(S.TLimit)==2
         ...
         error('Unknown ''TLimit'' Property Value.')
      end
   case 'TGridLineWidth'                                   % TGridLineWidth
      if isnumeric(S.TGridLineWidth) && isscalar(S.TGridLineWidth)
         ...
         error('Unknown ''TGridLineWidth'' Property Value.')
      end
   case 'TTickDelta'                                           % TTickDelta
      if isnumeric(S.TTickDelta) && isscalar(S.TTickDelta)
         ...
         error('Unknown ''TTickDelta'' Property Value.')
      end
   case 'TTickSign'                                             % TTickSign
      if ischar(S.TTickSign)
         ...
         error('Unknown ''TTickSign'' Property Value.')
      end
   case 'TTickScale'                                           % TTickScale
      ...
      elseif ~ischar(S.TTickScale)
         error('Unknown ''TTickScale'' Property Value.')
      end
   case 'TTickValue'                                           % TTickValue
      if isnumeric(S.TTickValue) && numel(S.TTickValue)>0
         ...
         error('Unknown ''TTickValue'' Property Value.')
      end
   ...
   case 'TTickLabel'                                           % TTickLabel
      if iscellstr(S.TTickLabel)
         ...
         error('Unknown ''TTickLabel'' Property Value.')
      end
   case 'TTickOffset'                                         % TTickOffset
      if isnumeric(S.TTickOffset) && isscalar(S.TTickOffset)
         ...
         error('Unknown ''TTickOffset'' Property Value.')
      end
   case 'TTickDirection'                                   % TTickDirection
      if ischar(S.TTickDirection) &&...
        (strcmpi(S.TTickDirection,'out') || strcmpi(S.TTickDirection,'in'))
         ...
         error('Unknown ''TTickDirection'' Property Value.')
      end
   case 'TTickLabelVisible'                             % TTickLabelVisible
      ...
      if istrue
         ...
         error('Unknown ''TTickLabelVisible'' Property Value.')
      end
   case 'TZeroDirection'                                   % TZeroDirection
      ...
      elseif ischar(S.TZeroDirection) && strncmpi(S.TZeroDirection,'west',1)
         ...
         error('Unknown ''TZeroDirection'' Property Value.')
      end
      P.Style='unknown';
   case 'Style'                                                     % Style
      ...
      elseif strncmpi(S.Style,'compass',3) % Compass style
         ...
         error('Unknown ''Style'' Property Value.')
      end
   end
end
setappdata(HAxes,'MMPOLAR_Properties',P)
...
D.RTickLabelN=length(P.RTickValue);
%--------------------------------------------------------------------------
function [P,D]=local_getTTickValue(P,D,S)             % local_getTTickValue
% Get Theta Ticks
...
%--------------------------------------------------------------------------
function [P,D]=local_placeAxesPatch(HAxes,P,D,S)     % local_placeAxesPatch
...
%--------------------------------------------------------------------------
function [P,D]=local_placeRGrid(HAxes,P,D,S)             % local_placeRGrid
tinc=pi/250;
if P.TLimit(1)>P.TLimit(2)
   theta=[P.TLimit(1):tinc:(P.TLimit(2)+2*pi-eps) P.TLimit(2)+2*pi];
else
...
for k=1:D.RGridN
   xdata=[xdata D.RTickRadius(k)*costheta NaN];
   ydata=[ydata D.RTickRadius(k)*sintheta NaN];
end
...
%--------------------------------------------------------------------------
function [P,D]=local_placeTGrid(HAxes,P,D,S)             % local_placeTGrid
xdata=[];
ydata=[];
...
D.TGridN=ke-ki+1;
for k=ki:ke
   xdata=[xdata 0 costheta(k) NaN];
   ydata=[ydata 0 sintheta(k) NaN];
end
if nargin<4 % new grid
...
%--------------------------------------------------------------------------
function [P,D]=local_placeTTickLabel(HAxes,P,D,S)   % local_placeTTickLabel
% Draw Theta Ticks
...
%--------------------------------------------------------------------------
function [P,D]=local_placeRTickLabel(HAxes,P,D,S)   % local_placeRTickLabel
