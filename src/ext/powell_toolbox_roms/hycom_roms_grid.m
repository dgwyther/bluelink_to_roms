...
% url='http://apdrc.soest.hawaii.edu:80/dods/public_data/Model_output/HYCOM/hycom_ncoda_hawaii_0.08/%s';
hycomfile=sprintf(url,'Latitude,Longitude,Depth');
% hycomlat=nc_varget(hycomfile,'Latitude',[ 0 0 ],[ 1 -1 ]);
hycomlat=nc_varget(hycomfile,'Latitude');
hycomlat=hycomlat(:,1);
hycomlon=nc_varget(hycomfile,'Longitude',[ 0 0 ], [-1 1]);
l=find(hycomlon>360);
hycomlon(l)=hycomlon(l)-hycomlon(1);
hycomdepth=-nc_varget(hycomfile,'Depth');
...
hycomfile=sprintf(url,'temperature');
hycomdat = squeeze(nanmean(nc_varget(hycomfile,'temperature', ...
...
if ( ~isempty(gwhich) )
  ...
  l=find(slon>180);
  slon(l)=slon(l)-360;
  lon_list=find( slon(1,:) >= min(grid.lonr(:)) &  ...
                 slon(1,:) <= max(grid.lonr(:)) );
  lat_list=find( slat(:,1) >= min(grid.latr(:)) &  ...
