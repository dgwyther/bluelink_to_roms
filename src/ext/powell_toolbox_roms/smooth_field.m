function field = smooth_field( field, varargin )

% surf = smooth_field( field, mask, dx, dimension] )
%
% This function will convolve ocean points with an averaging kernel.
%
% Example: you have a surface field(time,lat,lon) with a mask(lat,lon)
%     surf = smooth_field( field, mask, 5 )
%   would convolve a 5x5 kernel around all ocean points
%
...
count=field;
count(count~=0)=1;
num=permute(convn( permute(count,[2 3 1]), kernel ),[3 1 2]);
num=num(1:t,bounds(1):m+bounds(2),bounds(1):n+bounds(2));
surf=permute(convn( permute(field,[2 3 1]), kernel ),[3 1 2]);
