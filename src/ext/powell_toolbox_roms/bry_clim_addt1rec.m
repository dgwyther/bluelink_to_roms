...
rec=[];
tendprev=nc_varget(climprev,'temp_time');tendprev=tendprev(end);
...
for v=1:length(vars),
  ...
  ntime=length(nc_varget(climprev,'temp_time'));
   if dims(v)==3
  climp=nc_varget(climprev,var,[ntime-1 0 0],[1 -1 -1]);
   else
  climp=nc_varget(climprev,var,[ntime-1 0 0 0],[1 -1 -1 -1]);
   end
