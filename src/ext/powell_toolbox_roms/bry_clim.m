function bry_clim(grid,climfile,bryfile,epoch)
...
rec=[];
rec.bry_time = nc_varget(cfile,'temp_time');
...
for v=1:length(vars),
  ...
  if (dims(v) == 3)
    % north
    eval(sprintf('rec.%s_north = squeeze(clim(:,end,:)).*%c2north;', var, char(mask(v))));
    % south
    eval(sprintf('rec.%s_south = squeeze(clim(:,1,:)).*%c2south;', var, char(mask(v))));
    % east
    eval(sprintf('rec.%s_east = collapse(clim(:,:,end)).*%c2east;', var, char(mask(v))));
    % west
    eval(sprintf('rec.%s_west = collapse(clim(:,:,1)).*%c2west;', var, char(mask(v))));
  else
    % north
    eval(sprintf('rec.%s_north = squeeze(clim(:,:,end,:)).*%c3north;', var, char(mask(v))));
    % south
    eval(sprintf('rec.%s_south = squeeze(clim(:,:,1,:)).*%c3south;', var, char(mask(v))));
    % east
    eval(sprintf('rec.%s_east = collapse(clim(:,:,:,end)).*%c3east;', var, char(mask(v))));
    % west
    eval(sprintf('rec.%s_west = collapse(clim(:,:,:,1)).*%c3west;', var, char(mask(v))));
  end
  clear clim
end
