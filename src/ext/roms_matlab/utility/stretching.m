...
% New vertical coordinate using a quadratic function for the s-coord
% Adaptation proposed by Joao Souza - SOEST ; 2012
...
%   ds=1.0/N;
  if (kgrid == 1),
    for k=N:-1:1
        s(k)=-(k.^2 - 2*k*N+k+N^2-N)./(N^2-N)-0.01*(k.^2-k*N)./(1-N);
        disp([k s(k)])
    end
    Nlev=Np;
%     lev=0:N;
%     s=(lev-N).*ds;
    ...
    for k=1:N;
        k=k-.5;
        s(k+.5)=-(k.^2 - 2*k*N+k+N^2-N)./(N^2-N)-0.01*(k.^2-k*N)./(1-N);
    end
    Nlev=N;
%     lev=[1:N]-0.5;
%     s=(lev-N).*ds;
...
if (report),
  ...
  elseif (Vstretching == 5),
    disp(['Vstretching = ',num2str(Vstretching), '   New vert. coordinate (2012)']);
  end,
  if (kgrid == 1)
